...
for (const section of ['orders', 'products']) {
...
function renderAdminProducts() {
  ...
  tbody.innerHTML = products.map((p) => {
    ...
    const unit = osLang() === 'ar' ? p.unit_ar : p.unit_en;
    ...
    return `<tr>
      ...
      <td>${osEscape(category ? osT(category[1]) : p.category)}</td><td>${osFormatPrice(p.price)}</td>
...
document.getElementById('productEditorModal').addEventListener('hide.bs.modal', (event) => { if (osProductSaving) event.preventDefault(); });
